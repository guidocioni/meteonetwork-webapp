...
def main(plot_type='temperature_max', date_download=(datetime.now()-timedelta(1)).strftime(format='%Y-%m-%d'),
	...
	if plot_type == 'temperature_max':
		...
		plot_rain(projection, rain_sparse, rain, lons, lats, date_download, plot_filename)
	elif plot_type == 'gust':
		gust= data['wind max'].values
		gust_sparse = utils.filter_max_values(gust, lats, lons)
		plot_gust(projection, gust_sparse, gust, lons, lats, date_download, plot_filename)
	else:
...
def plot_temperature_max(projection, plot_type, temp_sparse, temp,
	...
	utils.add_logo_on_map(ax=plt.gca(), logo='meteoindiretta_logo.png', zoom=0.15, pos=(0.92, 0.1))
	utils.add_logo_on_map(ax=plt.gca(), logo='meteonetwork_logo.png', zoom=0.3, pos=(0.15, 0.05))
